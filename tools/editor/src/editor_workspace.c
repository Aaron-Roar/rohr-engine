...
static bool editor_workspace_main_write(const EditorWorkspace *workspace,
    const EditorProject *project) {
    char path[EDITOR_WORKSPACE_PATH_MAX * 2];
    char camera_object[EDITOR_OBJECT_NAME_MAX] = {0};
    const EditorCamera *main_camera = NULL;
    FILE *file;
    ...
    file = fopen(path, "wb");
    if(file == NULL) return false;
    for(size_t i = 0; i < project->object_count && main_camera == NULL; i += 1) {
        if(project->objects[i].camera_count == 0) continue;
        main_camera = &project->objects[i].cameras[0];
        editor_project_property_name_format(camera_object, sizeof(camera_object),
